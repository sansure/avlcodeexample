...
async function handleDebugLogin(request, env, ctx, url) {
  return withQuota(env, async (db, bucket, quota) => {
    const queryToken = url.searchParams.get("token");
    const cookies = parseCookieHeader(request.headers.get("Cookie"));
    const cookieToken = cookies[CONFIG.TOKEN_COOKIE];
    ...
    if (!db) {
      ...
      try {
        tableInfo = await db.prepare("SELECT COUNT(*) AS count FROM review_users").first();
      } catch (e) {
      ...
      if (token) {
        try {
          user = await db.prepare("SELECT id, name, role, token FROM review_users WHERE token = ?").bind(token).first();
        } catch (e) {
          error = e.message;
        }
      }
      ...
      try {
        const { results } = await db.prepare("SELECT id, name, role, token FROM review_users LIMIT 20").all();
        allUsers = results || [];
      } catch (e) {
    ...
    return json({
      ...
      queryToken,
      cookieToken,
      cookieRaw: request.headers.get("Cookie"),
      userFound: !!user,
      user: user ? { id: user.id, name: user.name, role: user.role, token: user.token } : null,
      tableInfo,
      allUsers,
      error,
      timestamp: nowIso(),
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/debug/login" && request.method === "GET") {
        return await handleDebugLogin(request, env, ctx, url);
      }
